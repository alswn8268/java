...
<body>
	...
	if (tempIdx != null && tempCurrentPage != null && tempIdx.trim().length() != 0 && tempCurrentPage.trim().length() != 0) {
		...
		try {
				...
				<form action="updateOK.jsp" method="post">
					<table width="700" align="center" border="1" cellpadding="5" cellspacing="0">
						...
						<tr>
							<td width="100">내용</td>
							<td width="600">
								<input type="text" name="memo" style="width: 97%" value="<%=rs.getString("memo").replace("<", "&lt;").replace(">", "&gt;")%>">
							</td>
						...
						<tr>
							<td colspan="2" align="center">
								<label>
									...
									<input class="button button1" type="submit" value="수정">
					...
					<!-- 수정할 글번호와 수정 후 돌아갈 페이지 번호도 updateOK.jsp로 넘겨줘야 한다. -->
			...
			out.println("<script>");
			out.println("alert('수정할 글 번호 또는 돌아갈 페이지 번호가 숫자가 아닙니다.')");		
			out.println("location.href='memoList4.jsp'");
			out.println("</script>");
		}	
	} else {
		// 수정할 글 번호와 돌아갈 페이지 번호가 모두 넘어오지 않아서 에러 메시지를 출력하고 memoList4.jsp의 1페이지로 돌려보냄
		out.println("<script>");
		out.println("alert('수정할 글 번호 또는 돌아갈 페이지 번호가 없습니다.')");		
		out.println("location.href='memoList4.jsp'");
		out.println("</script>");
	}
	%>
